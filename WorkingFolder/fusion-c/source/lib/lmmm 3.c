...
void	LMMM( unsigned int SX, unsigned int SY, unsigned int DX, unsigned int DY, unsigned int NX, unsigned int NY, char OP ) __naked
{
	SX,SY,DX,DY,NX,NY,OP;
__asm
...
; LMMM( unsigned int XS, unsigned int YS, unsigned int XT, unsigned int YT, unsigned int DX, unsigned int DY, char OP )
;****************************************************************
  ...
  ld a,16(ix)     ; OP
  and #0x0f
  or    #0b10010000   ;LMMM command  with LOGICAL
  out    (c),a      ;do it
  ei
  pop    ix
  ret
