...
typedef struct {
	  ...
	  unsigned int SectorSize;  				// Sector size (always 512 currently)
	  char SectorPerCluster;  					// Sectors per cluster (non-zero power of 2)
	  unsigned int NumberReservedSector;  		// Number of reserved sectors (usually 1)
	  ...
	  unsigned int TotalLogicalSectors;			// Total number of logical sectors
	  char MediaDescriptionByte;				// Media descriptor byte
	  char NumberSectorsPerFat;					// Number of sectors per FAT
	  unsigned int FirstRootSectorNumber;		// First root directory sector number
	  unsigned int FirstDataSectorNumber;		// First data sector number
	  unsigned int MaximumCluster;				// Maximum cluster number
...
extern 	void  			SetDiskTrAddress(void *address);  /* Set the transfert adress for Read/wrtie Sector */
extern  unsigned int  	GetDiskTrAddress(void); 					/* Get the current disk Transfer adress */
extern	char 			GetDiskParam(DSKPARAMS *info, char Drive); /*Get Disk current Parameters . Only for MSX-DOS2 */
